...
class C(BaseConstants):
    ...
    NUM_TRIALS = 10  # in the classic game it is 100
...
def live_method(player: Player, data):
    ...
    if 'letter' in data:
        ...
        #cost = session.costs[deck][cur_count]  # .Gambling_costs?
        payoff = reward #- cost
        player.payoff += payoff
        cur_count += 1
        setattr(player, field_name, cur_count)
        player.num_trials += 1
        resp.update(reward=reward) # , cost = cost
